...
This blog post walks through how RWKV works. RWKV is an interesting new
architecture for language models that combines nice features of
Transformers and recurrent networks.

There are other write-ups on how RWKV works, so why read this one?

-   Unlike the [arXiv paper](https://arxiv.org/abs/2305.13048), it uses
    code to explain the architecture, rather than equations. You can run
    it on Google Colab
    [here](https://colab.research.google.com/github/the-full-stack/website/blob/main/docs/blog/posts/rwkv-explainer/RWKV%2C_Explained.ipynb).

-   Unlike the (excellent) [blog post on RWKV by contributor Johan
    Wind](https://johanwind.github.io/2023/03/23/rwkv_details.html), it
    is written in PyTorch, rather than Numpy.

-   Unlike the [reference PyTorch
    implementation](https://github.com/BlinkDL/ChatRWKV/blob/main/RWKV_in_150_lines.py)
    which is written for concision (single-letter variable names,
    minimal comments) and robustness (numerical tricks), this
    implementation is written to be understood and sacrifices
    performance for clarity, e.g. including runtime type checking, but
...
re-read the whole sentence up to that point before it can proceed with
another.

But could it be different? Can we come up with an architecture that has
Transformers' non-negotiable parallelization at train time but retains
RNNs' favorable profile at inference time? And if we did, would it also
learn at least as much at least as quickly as a Transformer?

The [Receptance-Weighted Key-Value
architecture](https://arxiv.org/abs/2305.13048) (RWKV) is an answer in
the affirmative, confirmed up to the scale of 14B parameters and 331B
training tokens, or ~5x less than current SOTA for commerically
available language models at time of writing.

Through clever manipulation of exponentials, RWKV can be written in
either of two ways: "time-parallel mode" or "RNN mode". Essentially,
these exponentials look a bit like the softmax normalization in
Transformer attention in time-parallel mode but look like a
multiplicative decay on a memory in RNN mode.

So, with RWKV, we get to have our cake and eat it too: parallelizable
...
Let's dive in and see how RWKV inference works!

## Setup

This blog post is based on [an executable Colab
notebook](https://tfs.ai/rwkv-explainer-colab), so we've got to do a bit
of setup.
...
We're implementing this in PyTorch, a popular Python wrapper around fast
...
This is a tutorial, so our brains are the rate-limiting component.
...
For us, a *language* is a collection of sequences of *tokens* from a
*vocabulary* -- you might think of the tokens as letters from the
alphabet "vocabulary" forming sentences in the English language, or
bytes forming valid strings.

But the tokens for most capable language models, including this version
of RWKV, aren't quite so simple -- they are collections of letters that
...
So the `Tokenizer` handles the outermost interface for our model: it
translates between the strings that humans (and traditional software!)
like and our token identifiers.
...
Our token identifier is just a single integer.
...
And they work best when there are a lot of numbers aka a
high-dimensional array or `Tensor`.

PyTorch is giving us the tools we need to dynamically manipulate
`Tensor`s in Python using fast compiled C++ code.
...
wasteful, way to represent the token id `ii`: as a big `Tensor` with
`0`s everywhere except in the index `ii`, which is `1`.
...
But the best documentation is automatically checked so that it stays in
...
sequences. For example, they can predict which word is missing from a
sequence or which word might come next if it were to continue.
...
The `temperature` and `top_p` are included so that we can match the
typical generation interface for things like OpenAI and Claude, but the
details don't matter to us, so we've abstracted them behind the
`utils.sample` function.

The key to notice is that we're inheriting from `torch.nn.Module` here.
...
Right now, it's pretty abstract: it calls something called an
`embedding` that returns an `Embedding`, which it passes through a
method called `blocks` that returns a new `Embedding`. That final piece
is `unembed`ded to produce the `Probabilities` we need to fit the
...
So following our "inside-out" approach, we'll first define the
`embedding` and `unembedding`, in the next section, before we dive into
the RWKV-specific details.
...
But in their guts, neural networks are linear algebra machines, applying
dense tensors to dense tensors.

So the "interface" inside of our network is a dense tensor -- and one
much smaller than our vocabulary.

These dense tensors are often called `Embedding`s, because they take a
pointy object, like our collection of `OneHot` tensors that are all at
vertices of a big cube, and smush them together into a smooth object.

There are many other names for this piece of the interface, but we'll
stick with `Embedding` since that has taken off in the era of
embedding-based vector search, and we'll use `channel` to refer to an
individual dimension, because `EmbeddingDimension` is a mouthful.
...
That number is learned during training.

So we need to load the values from the weights -- which we do by adding
a janky class from our `utils`, a `LoadingMixin`:
...
It's nice for printing to terminal logs, but we're in a rich media
environment, so we can do better.

Let's use the `torchview` to get something cooler: a trace of all the
pieces of our model, to a variety of `depth`s.
...
1.  This module is a `TokenEmbedding` that takes in `50_277` dimensional
    tensors and returns `1024` dimensional tensors.
...
It's (up to normalization) equivalent to the [zero-layer
Transformer](https://transformer-circuits.pub/2021/framework/index.html#zero-layer-transformers).

It's entirely linear, so it's actually a type of logistic regression.
...
So "Drosophila melanosporum" is the scientific nomenclature equivalent
...
## Defining the Innermost Blocks: Gated MLP and Attention
...
added to the `Embedding` to produce the output.
...
The `LayerNorm`s look familiar, but what are those other layers?

For now, let's just put in placeholders, like we did for the RWKV blocks
in the zero-layer model, so that we can visualize our architecture again
...
the outputs of the last block are similar enough that the unembedding
head can read either.

It's also, like layer norm, important for stabilizing optimization.

Speaking of multiple blocks!

To fit this in our `AutoregressiveLM` interface, we'll combine multiple
`RWKVBlock`s into a single `Module` using `torch.nn.Sequential`.

We end up just calling them one after the other:
...
The `MLP` part is mostly standard and looks like the same part of but
uses non-standard nomenclature -- the input layer's weights are called
`key` weights and the output layer's weights are called `value` weights.
...
This is important for models where the meaningful sequences in the
language are made from multiple tokens, e.g. when you're working
directly with bytes.
...
We just need to explicitly assign a `torch.nn.Parameter` for our mixing
weights.
...
The memory is the heart of the architecture, and it's both pretty
different from other mainstream neural network components and fairly
complicated.

But, as a wise LM once said, ["let's work this out in a step-by-step way
to be sure we have the right answer"](https://arxiv.org/abs/2211.01910).

Let's start with the components, which we define in the `init`.
...
A `gain` is used to attenuate or amplify a signal. We'll use it on the
current embedding.
...
As part of a forwards pass, we update both, so we're doing some kind of
...
def step(self):
    ...
    decay_rate: ScalingWeight = exp(self.log_decay)  # exp ensures that decay rate is positive, so we forget the past
...
This ensures that the most recent value is treated differently than all
past values.
...
def forward(self, values: Embedding, importances: ScalingWeight) -> Update:
    ...
    # before adding it in and dividing to get the final memory "state" we report as output
    out = (self.contents + latest_contents) /  \
...
To really grok this architecture, I recommend pulling one up in a
separate window and mapping it onto the matching module's code.
...
information, so our model will just accumulate information forever.
...
Okay, so we can run a toy model with a few layers.
