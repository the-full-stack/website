...
The Receptance-Weighted Key-Value architecture, RWKV, is an answer in
...
available language models at time of writing. See the paper for details
on training and benchmarking against [equal-size Transformer
models](https://arxiv.org/abs/2305.13048).
...
richer `Embedding` that's got more information that's useful for
predicting the `NextTokenProbabilities`.
...
which expects just one `Module` (that maps `Embedding`s to `Embedding`s)
in the middle, we'll combine them using `torch.nn.Sequential`.
...
The `sigmoid` function turns the `receptances`, which can be any
floating point number, into `gating_values`, numbers that are between
`0` and `1`.
...
behavior out of a language model, so efficient-inference LMs have
already secured a spot in the language modeling tech stack!
